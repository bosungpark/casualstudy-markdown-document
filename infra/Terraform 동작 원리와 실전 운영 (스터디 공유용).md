...
| 도구 | 특징 |
|---|---|
| **Terraform Cloud Cost Estimation** | TFC 유료 플랜, plan 시 월 비용 자동 산출 |
| **Infracost** | OSS, CI 에서 PR 코멘트로 "이 변경의 월 비용 +$120" 표시 |
...
### 10.2 Drift detection 자동화

| 방식 | 동작 |
|---|---|
| **TFC Drift Detection** | 워크스페이스 단위로 주기적 health check, 알림 |
| **Cron + `plan -refresh-only`** | 직접 GitHub Actions 스케줄로 매일 plan, 변경 있으면 Slack |
| **driftctl** | OSS, "state 에 없는데 실제 존재" 까지 탐지 (shadow drift) |
  ...
  drift:
    ...
    steps:
      ...
      - if: steps.plan.outputs.exitcode == '2'
        run: ./notify-slack.sh "Drift detected"
```
...
- `terraform fmt` / `terraform validate` — 기본 린트.
- `tflint` — 클라우드별 베스트프랙티스 체크.
- `terraform-docs` — 모듈 문서 자동 생성.
- `checkov` / `tfsec` — 보안 정책 정적 분석.
...
terraform apply -refresh-only        # drift 동기화
...
> "알아서 실습" 항목에 채울 후보들. 1~2개만 골라 깊게.

1. **로컬 → S3 backend 마이그레이션**: `terraform init -migrate-state` 로 상태 옮기고 락 동작 확인 (`use_lockfile = true`).
2. **Drift 시뮬레이션**: AWS 콘솔에서 보안그룹 룰 추가 → `plan -refresh-only` 로 탐지 → import 로 흡수.
3. **EKS 한 줄 띄우기**: `terraform-aws-modules/eks/aws` + `helm_release` 로 ingress-nginx 까지 한 번에.
4. **GitHub Actions 파이프라인**: PR plan 코멘트 + main apply + Environments 승인 게이트 + Infracost.
5. **OPA/conftest 정책**: "모든 S3 버킷은 versioning + encryption" 정책을 plan JSON 에 적용.
6. **Drift detection cron**: 매일 09:00 `plan -refresh-only`, exitcode 2 면 Slack.
